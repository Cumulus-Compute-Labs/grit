#!/bin/bash
# FINAL TEST: --external 'mnt[]' for dump

PID=232300
CKPT_DIR=/tmp/criu-final-test
HOOK_SCRIPT=/tmp/criu-fix-mounts.sh

echo "=== FINAL TEST: --external 'mnt[]' ==="
...
echo "Command: criu dump -t $PID -D $CKPT_DIR --external 'mnt[]' ..."
echo ""
...
cat /tmp/criu-action.log 2>/dev/null | tail -10
echo ""
