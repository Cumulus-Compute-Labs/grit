#!/bin/bash
# Test CRIU with --external mnt[] to skip NVIDIA GPU mounts

PID=232300
CKPT_DIR=/tmp/criu-external-mnt-test
HOOK_SCRIPT=/tmp/criu-fix-mounts.sh

echo "=== Testing CRIU with --external mnt[] ==="
...
if [ "$CRTOOLS_SCRIPT_ACTION" == "pre-dump" ]; then
    ...
    # Find and make private all slave mounts
    ALL_SLAVE_MOUNTS=$(awk '{
        for (i=1; i<=NF; i++) { if ($i == "-") break; }
        for (j=7; j<i; j++) {
            if ($j ~ /^master:/) { print $5; break; }
        }
...
# CRIU dump with --external mnt[]
echo "=== Step 3: CRIU Dump with --external mnt[] ==="
sudo rm -rf $CKPT_DIR
sudo mkdir -p $CKPT_DIR

echo "Running CRIU with:"
echo "  -t $PID"
echo "  --external 'mnt[/proc/driver/nvidia]:m'"
echo "  --action-script $HOOK_SCRIPT"
echo ""
...
sudo /usr/local/bin/criu.real dump \
    ...
    --tcp-established \
    --ext-unix-sk \
    --external 'mnt[/proc/driver/nvidia]:m' \
