#!/bin/bash
# Full checkpoint/restore test with descriptors.json fix - v2
...
for i in {1..60}; do
    ...
    if [ "$STATUS" = "Running" ] && [ -n "$POD_NAME" ]; then
...
echo ""
echo "=== Step 5: Create checkpoint with actual pod name ==="
cat <<EOF | kubectl apply -f -
...
echo ""
echo "=== Step 7: Check checkpoint files ==="
echo "Files in checkpoint dir:"
ls -la /mnt/grit-agent/default/gpu-test-ckpt/ 2>/dev/null || true
echo ""
echo "Files in cuda/checkpoint:"
ls -la /mnt/grit-agent/default/gpu-test-ckpt/cuda/checkpoint/ 2>/dev/null | head -20

echo ""
echo "=== Step 8: Verify descriptors.json exists ==="
if [ -f /mnt/grit-agent/default/gpu-test-ckpt/cuda/checkpoint/descriptors.json ]; then
    echo "descriptors.json exists!"
    cat /mnt/grit-agent/default/gpu-test-ckpt/cuda/checkpoint/descriptors.json
else
    echo "descriptors.json MISSING!"
fi

echo ""
echo "=== Step 9: Get ReplicaSet UID for restore ==="
...
echo ""
echo "=== Step 10: Scale down and create restore ==="
...
echo ""
echo "=== Step 11: Scale up to trigger restore ==="
sleep 2
kubectl scale deployment gpu-test --replicas=1

echo ""
echo "=== Step 12: Monitor restore ==="
for i in {1..40}; do
...
kubectl logs -l app=gpu-test --tail=20 2>/dev/null || echo "No logs"
