#!/bin/bash
# Test CRIU with --skip-mnt to ignore NVIDIA GPU mounts

PID=232300
CKPT_DIR=/tmp/criu-skip-mnt-test
HOOK_SCRIPT=/tmp/criu-fix-mounts.sh

echo "=== Testing CRIU with --skip-mnt ==="
...
# CRIU dump with --skip-mnt
echo "=== Step 3: CRIU Dump with --skip-mnt ==="
sudo rm -rf $CKPT_DIR
sudo mkdir -p $CKPT_DIR

echo "Running CRIU with --skip-mnt /proc/driver/nvidia/gpus"
echo ""
